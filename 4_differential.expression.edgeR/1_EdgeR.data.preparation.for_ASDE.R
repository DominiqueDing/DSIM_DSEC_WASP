...
counts.y<-counts%>%select(-SC_sig_DE.cat)
...
counts.y$counts.ID<-paste(counts.y$lib,counts.y$species,counts.y$treatment,counts.y$tissue,counts.y$allele,sep = '.')
counts.y$group.ID<-paste(counts.y$species,counts.y$treatment,counts.y$tissue,counts.y$allele,sep = '.')
...
write.csv(x = y.counts,file = "edgeR.ASDE.data.csv")
