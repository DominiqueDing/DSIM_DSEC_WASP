...
####################################### creating DGEList ##########
###################################################################
y.haemocytes <- DGEList(counts = y.counts,group = substr(x=names(y.counts),start = 5,stop = 17))
# remove(y.counts)
keep<-rowSums(cpm(y.haemocytes)>3) >= 3
table(keep)
# keep
# FALSE  TRUE 
# 2199  5125
y.haemocytes<-y.haemocytes[keep, , keep.lib.sizes=FALSE]
y.haemocytes<-calcNormFactors(y.haemocytes)
### MD plot: column=sample(i)
plotMD(cpm(y.haemocytes, log=TRUE), column=2)
abline(h=0, col="red", lty=2, lwd=2)
write.table(x = rownames(y.haemocytes$counts),file = "ASDE.analysed.genes.for.haemocytes.txt",quote = F,row.names = F,col.names = F)

###################################################################
################# design ##########################################
###################################################################
design <- model.matrix(~ 0 + group, data = y.haemocytes$samples)
colnames(design) <- levels(y.haemocytes$samples$group)
### Estimating the dispersion:
y.haemocytes <- estimateDisp(y.haemocytes, design, robust = TRUE)
y.haemocytes$common.dispersion
# [1] 0.1125753
plotBCV(y.haemocytes)
### fit quasi-likelihood model
fit<-glmQLFit(y.haemocytes,design = design,robust=TRUE)
plotQLDisp(fit)
...
write.csv(x = ana.ctrl, file = "ASDE.result.hemo.ctrl.csv",row.names = F)
write.csv(x = ana.wasp, file = "ASDE.result.hemo.wasp.csv",row.names = F)
write.csv(x = ana.reac, file = "ASDE.result.hemo.reac.csv",row.names = F)
...
write.csv(x = ana.DE, file = "ASDE.result.hemo.response.within.SIM.SEC.csv",row.names = F)
