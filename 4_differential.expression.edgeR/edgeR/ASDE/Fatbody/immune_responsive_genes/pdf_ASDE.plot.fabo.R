...
ana.ctrl <- read.csv("ASDE.result.fabo.ctrl.csv")
ana.wasp <- read.csv("ASDE.result.fabo.wasp.csv")
ana.reac <- read.csv("ASDE.result.fabo.reac.csv")
DE.within <- read.csv("DE.result.fabo.response.within.SIM.SEC.HYB.csv")
...
pdf("cis.trans.log.plot.fabo_immune.responsive.genes.pdf",width = 15,height = 6.3)
...
pdf("cis.trans.bar.plot.fabo_immune.responsive.genes.pdf",width = 15,height = 2.5)
...
pdf('fabo.cis.trans.number.genes.hist.plot_immune.responsive.genes.pdf',width = 15,height = 5)
ggplot(data = data.hist%>%mutate(logFC.total=abs(logFC.total)))+
  geom_histogram(aes(x = logFC.total,fill = Cat.main),colour='black',position = position_dodge(),bins = 17)+
