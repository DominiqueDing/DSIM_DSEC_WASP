...
# import genes:
genes<- read.table("ASDE.analysed.genes.for.fatbody.txt",col.names = "gene")
...
write.csv(x = cpm(y),file = "within.cpm.fabo.csv")
###################################################################
################# design ##########################################
###################################################################
...
write.csv(x = result.within.DE, file = "DE.result.fabo.response.within.SIM.SEC.HYB.csv",row.names = F)

result.h.sim <- bind_rows(df.result_DE.h.sim.c%>%mutate(treatment='Control'),
                          df.result_DE.h.sim.w%>%mutate(treatment='Wasp'),
                          df.result_DE.h.sim.r%>%mutate(treatment='Response'))%>%mutate(tissue='Fat Body')
result.h.sec <- bind_rows(df.result_DE.h.sec.c%>%mutate(treatment='Control'),
                          df.result_DE.h.sec.w%>%mutate(treatment='Wasp'),
                          df.result_DE.h.sec.r%>%mutate(treatment='Response'))%>%mutate(tissue='Fat Body')
...
write.csv(x = result.between.DE, file = "DE.result.fabo.response.betweenHSim.betweenHsec.csv",row.names = F)

result.sim.sec <- bind_rows(df.result_DE.sim.sec.c%>%mutate(treatment='Control'),
                            df.result_DE.sim.sec.w%>%mutate(treatment='Wasp'),
                            df.result_DE.sim.sec.r%>%mutate(treatment='Response'))%>%mutate(tissue='Fat Body')
write.csv(x = result.sim.sec, file = "DE.result.fabo.response.betweenSIMSEC.csv",row.names = F)
